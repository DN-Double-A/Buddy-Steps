...
export function EditTask(props) {
  ...
  const emptyForm = { updatedTask: new Date(), updatedEndTime: new Date(), taskID: null }
  const [formData, setFormData] = useState(emptyForm);

  //& On rendering the Edit task, need to display current task data
  useEffect(() => {
    if (taskIndex != -1) { //on the very first render before user even gets a chance to press the edit button. we dont want to run the code below. test task index to a default value of -1 in taskboard.jsx
      // console.log(taskData[taskIndex])
      // console.log(typeof taskData[taskIndex].enddate)
      setFormData({ updatedTask: taskData[taskIndex].task, updatedEndTime: taskData[taskIndex].enddate, taskID: taskData[taskIndex].taskID })
    }
  }, [taskIndex])
            ...
            <div className='edit-task-popup-inner'>
              ...
              <form onSubmit={handleSubmit} className='form'>
                <div>
                  ...
                  <input type='text' id='text' name='updatedTask' value={formData.updatedTask} onChange={handleChange}></input>
                </div>
                <div>
              ...
              <button className='edit-task-close-button' onClick={closeEditPopup} >X</button>
            </div>
          </div>
          : ""
      }
